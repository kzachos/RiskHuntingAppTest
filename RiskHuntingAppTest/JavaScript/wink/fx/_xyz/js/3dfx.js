...
(function(winkfx)
{
	 ...
	 */
	winkfx.set3dTransform = function(node, transformation, keepCurrent)
	{
	 ...
	 *	--> keepCurrent: true if the previous node transformation must be kept
	 */
	winkfx.initComposedTransform = function(node, keepCurrent)
	{
		var cssTransform;
		if (keepCurrent === true)
		{
		...
		var referenceMatrix = wink.math.createTransformMatrix(cssTransform);
		winkfx.removeComposedTransform(node); // to ensure unicity
	 ...
	 *	--> transformation: the 3dfx transformation
	 */
	winkfx.setTransformPart = function(node, index, transformation)
	{
		if (!_is3dfxTransformation(transformation))
		{
			wink.log('[3dfx] Invalid transformation : check type, x, y, z, angle parameters');
	 ...
	 */
	winkfx.applyComposedTransform = function(node, store)
	{
		var i, list = _nodeTransforms, l = list.length;
		for (i = 0; i < l; i++)
		{
	 ...
	 *	--> node: the node that hosts the composed transformation
	 */
	winkfx.storeComposedTransform = function(node)
	{
		winkfx.applyComposedTransform(node, true);
	};
	/**
	 ...
	 *	--> node: the node that hosts the composed transformation
	 */
	winkfx.removeComposedTransform = function(node)
	{
		var i, list = _nodeTransforms, l = list.length;
		for (i = 0; i < l; i++)
		{
