...
//Rico.animate({
//  fadeOut: $('message'), 
//  width: {element: $("message"), to: 244}
//})

//Rico.animate({fadeIn: element})
//Rico.animate({width:element, to:255}, {height:element, to:223})

//Rico.animate({fadeOut:{element:$('message'), }})
//Rico.animate({slideUpClose:element})
...
Rico.Effect.FadeOut.prototype = {
  ...
  step: function(framesLeft) {
    this.opacity = this.opacity - this.opacity/framesLeft;
    Rico.Effect.setOpacity(this.element,Math.max(0,this.opacity));
  }
}
...
Rico.Effect.FadeIn.prototype = {
  initialize: function(element){
    ...
    Rico.Effect.setOpacity(this.element, startValue);
  },
  step: function(v) {
    var curOpacity = parseFloat(Element.getStyle(this.element, 'opacity'));
    var newOpacity =  Math.min(curOpacity + (1.0 - curOpacity)/v, 1.0);
    Rico.Effect.setOpacity(this.element, Math.max(0,newOpacity));
  },
  finish: function(){}
}
